...
const inputString = `Group stage 
  ...
  16 June
  D: Poland 1-2 Netherlands (Hamburg, 15:00)
  C: Slovenia 1-1 Denmark (Stuttgart, 18:00)
  C: Serbia 0-1 England (Gelsenkirchen, 21:00)
  17 June
  E: Romania 3-0 Ukraine (Munich, 15:00)
...
const parseDate = (day: string, month: string, time: string): Date => {
    ...
    const date = new Date(2024, monthMap[month], parseInt(day), parseInt(time.split(':')[0])-1, parseInt(time.split(':')[1]));
    return date;
};
